...
// precedence chain, so `ellipsis auth login` alone can't replace it.
export function friendlyErrorMessage(err: unknown): string {
  if (err instanceof APIError && err.status === 401) {
    return envToken()
      ? 'The server rejected ELLIPSIS_API_TOKEN. Check the token, or unset it and run `ellipsis auth login`.'
      : 'Your login is invalid or has expired. Run `ellipsis auth login` to re-authenticate.'
  }
...
function upgradeHint(): string {
  return (
    ...
    'Check for a newer CLI by running: ellipsis update'
  )
}
